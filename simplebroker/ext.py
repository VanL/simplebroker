...
Backend API v5 extends exact broadcast selection with
``BrokerConnection.broadcast(..., queue_names=..., create_missing=True)``.
Direct backends must preserve existing-only selection by default and insert
