"""Retry and setup-progress policy for SimpleBroker.

``_retry.py`` owns the retry *mechanism* -- stop conditions, wait strategies,
and the execution loop. This module owns the *policy* built on it: which
errors are retryable, the setup-phase budgets, and the progress-stall guard
that keeps a contended setup from spinning without advancing.
"""
