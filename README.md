...
### Writing a message with a generated ID

Use `write_reserved_message(...)` when your application needs to generate the
message ID before the message body is ready. This is for protocols where the
message ID is part of the domain contract.
...
The supplied `message_id` must have already been generated by the destination
broker, so it must be less than or equal to the current `last_ts`. SimpleBroker
also enforces global uniqueness for the ID. It cannot prove that the caller
"owns" an old unused ID below `last_ts`, so do not use this API for dump
restore.
...
be lower than the broker's current `last_ts`.

```python
from simplebroker import open_broker

with open_broker("/path/to/.broker.db") as broker:
    broker.import_message("tasks", "restore one", message_id=1837025672140161024)
```

For dump/load, use `import_messages(...)`. It validates the full batch, advances
`last_ts` above the largest imported ID inside the same transaction, and then
imports the pending messages with their exact IDs. That lets a fresh destination
load historical rows without a priming write:
...
with open_broker("/path/to/.broker.db") as broker:
    broker.import_messages(records)
```

There is no CLI surface for exact-ID writes or imports.
...
then creates the GitHub release with built artifacts. Core releases wait for
`Test`, `Test Postgres Extension`, and `Test Redis Extension`; extension
releases wait for `Test` plus their matching backend workflow.
