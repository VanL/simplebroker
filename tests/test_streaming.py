"""Test streaming functionality for memory efficiency."""

import json
import threading
from contextlib import redirect_stdout
from io import StringIO
from pathlib import Path

import pytest

from simplebroker import Queue, target_for_directory
from simplebroker._constants import PEEK_BATCH_SIZE
from simplebroker.commands import cmd_read

from .conftest import run_cli


def _bulk_write(workdir: Path, queue_name: str, count: int) -> None:
    """Seed messages through the Python API, backend-agnostically.

    ``target_for_directory(workdir)`` resolves the same broker the CLI
    subprocesses resolve in *workdir* (the SQLite default db, or the pg/redis
    project config the worker fixtures wrote there), so seeding takes one
    connection instead of one subprocess per message. This is what lets these
    tests run in the regular suite instead of being marked slow.
    """
    target = target_for_directory(workdir)
    q = Queue(queue_name, db_path=target)
    try:
        for i in range(count):
            q.write(f"message_{i}")
    finally:
        q.close()


@pytest.mark.windows_serial
@pytest.mark.timeout(360)
@pytest.mark.sqlite_only
def test_read_all_emits_before_source_exhaustion(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    requested_second = threading.Event()
    release_second = threading.Event()

    def controlled_rows(self, **kwargs):
        del self, kwargs
        yield "first", 1234567890123456789
        requested_second.set()
        assert release_second.wait(timeout=2)
        yield "second", 1234567890123456790

    monkeypatch.setattr(Queue, "read_generator", controlled_rows)
    sink = StringIO()
    result: list[int] = []

    def run_command() -> None:
        with redirect_stdout(sink):
            result.append(cmd_read(str(tmp_path / "stream.db"), "q", all_messages=True))

    worker = threading.Thread(target=run_command)
    worker.start()
    try:
        assert requested_second.wait(timeout=2)
        assert sink.getvalue() == "first\n"
    finally:
        release_second.set()
        worker.join(timeout=2)

    assert not worker.is_alive()
    assert result == [0]
    assert sink.getvalue() == "first\nsecond\n"


@pytest.mark.windows_serial
@pytest.mark.timeout(360)
def test_peek_all_crosses_page_boundary_without_mutation(workdir: Path) -> None:
    """peek --all paginates past one internal page and leaves rows pending."""
    count = PEEK_BATCH_SIZE + 1
    _bulk_write(workdir, "test_queue", count)

    code, stdout, stderr = run_cli(
        "peek", "test_queue", "--all", cwd=workdir, timeout=120.0
    )
    assert code == 0, stderr

    lines = stdout.strip().split("\n")
    assert len(lines) == count
    assert lines[0] == "message_0"
    assert lines[count - 1] == f"message_{count - 1}"

    with Queue("test_queue", db_path=target_for_directory(workdir)) as queue:
        assert list(queue.peek_generator()) == [
            f"message_{index}" for index in range(count)
        ]


def test_json_output_now_ndjson(workdir: Path) -> None:
    """Test that JSON output now uses line-delimited JSON (ndjson) format."""
    for i in range(5):
        code, _, _stderr = run_cli("write", "test_queue", f"message_{i}", cwd=workdir)
        assert code == 0

    code, stdout, _stderr = run_cli(
        "read", "test_queue", "--all", "--json", cwd=workdir
    )
    assert code == 0

    lines = stdout.strip().split("\n")
    assert len(lines) == 5

    for i, line in enumerate(lines):
        data = json.loads(line)
        assert "message" in data
        assert data["message"] == f"message_{i}"
