...
| Plans | Soft-retirement 2026-09-14: 6 of 7 completed plans → `retired-pending`; the message-ID order plan remains harvest-blocked | plan sources `4efe7b3` (published) and `1c6898b` (local-only); prior 5-plan deletion sources remain in the Retired Plans ledger |
| Promotion | (none) | — |
...
|------------|------:|------------------------|------------|
| 2026-07-30..2026-08-13 (noncontiguous; 23 cold entries retained) | 27 | Executable inventories and default-path proof → Golden Rules, engineering principles §11/12, writing-specs, testing-patterns, and traceability; plan verification and negative knowledge → review-loops, hardening, writing-plans, [DOM-16], and retained plan sources; product atomicity, lifecycle, worker, and dump facts → specs 10/13/16/17, implementation docs 06–10, and executable examples. | `5f69748` (published on `origin/main`) |
| 2026-07-27..2026-07-28 | 10 | Plan status, supersession, and complete-index rules → final Golden Rule, writing-plans lifecycle, and this file's derivation contract; cross-thread finalization → `[REV-THEORY-005]`, `[SB-DELIVERY-6]`, and implementation doc 04; ID-preserving move/checkpoint and claim-versus-processing safety → `[REV-THEORY-003]`, `[SB-ID-5]`, `[SB-SELECT-2/3]`, `[SB-DELIVERY-1/3/5]`, and implementation docs 08/09; backend split → `[THEORY-2/3]`, `[SB-API-11]`, and implementation doc 06; phase-lock facts → implementation docs 07/09 plus migration-wait evidence retained at source `88466aff`; coverage reliability → the coverage Golden Rules; zero-target Redis allocation → `[SB-BCAST-4]` and `SM-REDIS-BROADCAST`. | `813dd7ce` |
...
|------|------------------------------|-----------------|-----------------|-----------------|
| Lessons | 2026-09-14, coalescing snapshot text/symbol review | 51 dated ledger entries; 23 are at least 30 days old | Added one hot release-gate lesson on separating transaction-owned fixture setup from measured behavior. Folded no cold entry: the prior 23 remain active themes or live platform/workflow facts without a verified topic owner, and the causal-proof reconsideration date has not fired. Chronological watermark stays at 2026-07-28 because no raw entry was removed. | Reconsider the causal-proof cluster on 2026-09-24; also reconsider when the remaining platform facts gain a verified topic owner |
| Plans | 2026-09-14, published source `4efe7b3`, local-only source `1c6898b`, and prior source assignments in the Retired Plans ledger | harvest_candidates=1; retired-pending=6; status-review=1; active=0 | Soft-retired six completed plans after five-gate audits. The Config simplification successor resolved the shared-loader compatibility-view blocker by promoting the owner-selected no-view contract. Kept the message-ID order plan completed because its rejection of permanent dual SQL layouts still lacks the required promoted rationale or explicit owner judgment. | Promote the message-ID dual-layout alternative under its required planning/owner gate, or obtain an explicit owner judgment that it is plan-local and non-durable; reconsider when a new completed plan appears |
| Unindexed | 2026-09-14, coalescing snapshot census | 0 | All eight plan files are indexed: six `retired-pending`, one completed/harvest-blocked, and one `status-review`. | unindexed &gt; 0 |
| Promotion | 2026-09-14, current-tree citation review | one mature uncaptured causal-concurrency proof theme; no promotion made | No new independent promotion evidence changed the 2026-09-12 judgment. The theme remains under its 2026-09-24 reconsideration hold, and durable-guidance promotion still requires an owner-ratified Class 3+P plan. | Reconsider on 2026-09-24, or earlier if the owner authorizes the guidance plan |
...
|------|---------|------------|-------|
| 2026-09-14 | plans + lessons + promotion | five plans at published `4efe7b3`; verified-review plan at local-only `1c6898b` | **Soft-retired 6 of 7 completed plans after isolated-snapshot five-gate audits.** Converted every spec backlink for the verified-review and shared-loader plans, retained all six files as `retired-pending`, and added source-pinned ledger outcomes. The Config simplification successor and current `[SB-API-2]`/implementation boundaries resolved the shared-loader compatibility-view blocker. The message-ID order plan remains completed and harvest-blocked because its durable rejection of permanent dual SQL layouts still lacks a steady-state rationale record or explicit owner judgment. Added one dated lesson from the release recovery: transaction-own bulk fixture setup before measuring behavior. Folded no cold lesson and made no durable-guidance promotion; the 23 cold retained entries and causal-proof theme remain deferred until their recorded conditions fire. Post-run snapshot: harvest=1, retired-pending=6, status-review=1, active=0, unindexed=0, dated lessons=51; apparatus share 0/2. Verification: `bin/check-doc-paths` OK; `bin/check-plan-context` OK (1 in-flight); `python3 bin/check-dom15-fixtures` OK; `bin/coalesce-check` OK (33 SHA claims, 4 foreign informational, local-only `1c6898b`/`f213b12`/`4d11cbc`/`f133ce7`/`7409242`, 51 dated lessons); doc/plan-context selection 8 passed; `git diff --check` passed. |
| 2026-09-14 | Plans (deletion follow-up) | soft-retirement recorded 2026-09-12; published plan sources `3868d7a`, `5f69748`, `646718f`, `dbace84`, and `3418079` | **Physically deleted all 5 `retired-pending` plans** after rechecking the current-tree five-gate harvest results. Each source is retrievable, reachable from `origin/main`, and byte-identical to the pre-deletion file; none defines a promoted `[ALT-*]` record. Repaired the keyset plan's orphaned local cue by repinning its identical published final state at `3868d7a`, updated its retired backlinks, and converted the sole remaining live-plan citation to a source-qualified historical citation. Removed the five Status Index rows and retained their Retired Plans ledger records. No lesson or durable guidance changed. Post-run deletion-wave snapshot: harvest=7, retired-pending=0, status-review=1, active=0, unindexed=0, dated lessons=50. Verification on the isolated landing snapshot: `bin/check-doc-paths` OK; `bin/check-plan-context` OK (1 in-flight); `python3 bin/check-dom15-fixtures` OK; `bin/coalesce-check` OK; `git diff --check` passed. |
| 2026-09-12 | plans + lessons + promotion | lessons and four plans at published `5f69748`/`646718f`/`dbace84`/`3418079`; keyset plan repinned 2026-09-14 to identical published source `3868d7a` | **Soft-retired 5 of 7 completed plans after parallel independent five-gate audits and current-tree reproduction.** Converted all spec and implementation backlinks to source-pinned retired citations and retained all five files as `retired-pending`. The shared-config plan remains completed and harvest-blocked because dropping legacy/namespaced compatibility views is durable public-contract negative knowledge without a steady-state alternative record. The message-ID plan remains completed under the prior durable dual-layout block; the contrary review judgment did not satisfy the recorded owner gate. **Repaired the lessons derivation surface:** two revised bullets had placed revision text before the required colon and were invisible to the authoritative prefix grammar, so the pre-fold semantic count was 76 rather than 74. **Folded 27 noncontiguous cold lessons** already owned by standing guidance, product/implementation docs, executable examples, or retained plan sources; the published retrieval cue and owner map survive in Lesson Fold Records. Kept 23 cold entries for accumulating themes, missing topic owners, or current-tree parity blockers; watermark remains 2026-07-28. Promotion checked-deferred: a three-plus-citation causal-concurrency proof theme exists but is still accumulating and requires an owner-ratified Class 3+P plan; reconsider 2026-09-24. Repaired Retired Plans absorption claims that still named the folded raw lessons. No plan file was deleted and no durable guidance was revised. Post-run: harvest=2, retired-pending=5, status-review=1, active=0, unindexed=0, dated lessons=49; apparatus share 0/3. Verification: `bin/check-doc-paths` OK; `bin/check-plan-context` OK (1 in-flight); `python3 bin/check-dom15-fixtures` OK; `bin/coalesce-check` OK (31 SHA claims, 4 foreign informational, local-only `f213b12`/`4d11cbc`/`f133ce7`/`7409242`, 49 dated lessons); doc/plan-context selection 8 passed; `uv run --locked pytest` 3501 passed, 18 skipped; `git diff --check` passed; independent post-edit review findings incorporated. |
