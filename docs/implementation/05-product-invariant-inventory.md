...
| Message identity, allocation, exact-ID handling, and preservation | Hybrid ID representation/allocation; zero reserved for lower-bound origin while legacy selectors stay broad; ordinary-write allocation/insertion visibility; write returns; broker-global high-water/cache meaning; exact-ID insertion; move preserves ID | `docs/specs/13-message-identity-contract.md`; `docs/implementation/08-message-identity-and-write-visibility.md`; README and agent-kernel restatements | `SB-ID-*` | `tests/test_message_identity_contract_sb_id.py`; shared reserved-zero, timestamp, write-return, SQL transaction, exact-ID, insert, cache, and move suites; real-Valkey stale-fence, visibility, monotone-resync, and `SM-REDIS-WRITE` suites | `canonical-spec` |
| Ordered timestamp selection and filter consequences | Strict open bounds as filters; late older ids under a bound; watch progress | `14-timestamp-selection-contract.md` `[SB-SELECT-*]`; CLI string forms `[SB-CLI-5]` | `SB-SELECT-*` | `tests/test_timestamp_selection_contract_sb_select.py`; `tests/test_after_flag.py` | `canonical-spec` |
| Dump/load / claimed | Dump/load v1 format; pending-only dump; fresh load; claimed inspection | `15-persistence-io-contract.md` `[SB-IO-*]` | `SB-IO-*` | `tests/test_persistence_io_contract_sb_io.py`; `tests/test_dump_load.py` | `canonical-spec` |
