...
|--------|---------------|-------|----------------|---------------------|-------|
| Delivery / claim / peek-stream | Consume claim-before-process; preferred move-to-inflight; no delete-while-peek-stream; generators thread-affine | `docs/specs/11-delivery.md`; README and agent-kernel restatements | `SB-DELIVERY-*` | `tests/test_delivery_contract_sb_delivery.py` derives the exact clause inventory and README/kernel terminal ranges; delivery CLI/API suites; backend finalization probes | `canonical-spec` |
