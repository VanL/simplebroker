...
| `SM-SQLITE-RUNNER` (confirmed) | `simplebroker/_runner.py::SQLiteRunner` | runner lifecycle table beside `tests/test_runner_error_handling.py` | runner error, fork-safety, schema, and setup suites | Per-thread connection, process identity, setup marker, borrowed/owned resources, and closed state govern later run and cleanup calls. |
...
| `SM-REACTOR-OUTPUT` (confirmed) | `examples/reference_reactor.py` pending-output storage and publisher | `examples/tests/test_reference_reactor.py` | pending-output replay, retry, route-drift, backlog, and crash-restart tests | Durable pending output governs publication, retry, replay, success, and terminal route-error actions across restarts. |
