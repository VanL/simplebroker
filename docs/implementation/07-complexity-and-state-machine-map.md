...
| `SM-PROCESS-SESSION` (confirmed) | `simplebroker/_broker_session.py::_ProcessBrokerSession` and `_ProcessBrokerSessionRegistry` | `tests/test_connection_transition_tables.py` | `tests/test_process_broker_session.py`; held-lock fork/control in `tests/test_fork_safety.py` | Session lifecycle, a boolean pending explicit-cleanup request, operation depth and manager stack publication, one active-disposal hold, and runner leases constrain connection creation, reuse, caller-thread release, and post-close calls across threads. Queue close releases its lease only; each thread retains its cached core until explicit cleanup on that thread or final session shutdown. Explicit cleanup defers through the outermost operation, failed nested acquisition cannot consume its outer operation, and an unfinished disposal claim returns to session ownership only while the session remains live. A failure returning after terminal timeout surfaces to its disposer without closed-session retry. Registry PID recovery precedes acquire/release/shutdown locks; child recovery retains inherited entries without finalization and starts empty. DBConnection checks its session-key PID before project setup/admission and ignores inherited cleanup leases; SQL rejects while Redis obtains a child session, proved through real fork in `tests/test_fork_safety.py` and `extensions/simplebroker_redis/tests/test_redis_pool.py`. Public `BrokerSession` handles acquire through this registry and add no parallel identity or cache. The separate retry diagnostic guard in `_retry.py` follows the same pre-lock PID rule, proved in `tests/test_retry.py`. |
...
| `SM-POLLING` (confirmed) | `simplebroker/watcher.py::PollingStrategy` | `tests/test_watcher_transition_tables.py::POLLING_TRANSITIONS` | watcher, burst-mode, edge-case, stop, race, PostgreSQL notify, and Redis integration suites | Waiter identity, burst/backoff phase, activity hints, stop state, one optional native deadline, and one cross-context local-notification latch persist across waits and callbacks. `notify_activity()` only arms the coalescing latch; the serialized wait owner publishes the drain hint and applies cadence, burst, and native safety-recheck state. A caller deadline bounds only the internally looping native branch and leaves SQLite's ordinary configured pass unchanged. As required by `[SB-API-6]`, all four direct constructor defaults come from one ambient-free canonical config snapshot; `BaseWatcher` passes its retained resolved values explicitly. |
...
| `SM-WATCHER-LIFECYCLE` (confirmed) | `simplebroker/watcher.py::BaseWatcher` | `WATCHER_LIFECYCLE_TRANSITIONS` in `tests/test_watcher_transition_tables.py` | transition-table, stop, error, and concurrency suites; exact run-thread cache and owned/supplied Queue lease cases in `tests/test_watcher_cleanup.py` | Thread state, Queue lease ownership, waiter attachment, retry state, terminal exception propagation, stop state, the active consume iterator, and the lock-protected cleanup owner govern legal start, run, stop, join, and cleanup calls. Consume mode checks stop before every explicit `next()` and closes its iterator exactly once on the advancing thread. Ordinary close failures are terminal with no active failure, remain note evidence during retryable or error-handler failures, and cannot be swallowed by clean-stop handling; a close `BaseException` retains lifecycle priority. Run or stop-before-run claims cleanup before blocking work; join timeout does not transfer ownership, and runtime cleanup failure reopens an explicit retry path. Run exit releases the run thread's cache regardless of Queue ownership and closes an internally constructed Queue's lease; idle stop closes the strategy and an internally constructed Queue's lease without releasing the stop caller's cache. A caller-supplied Queue's lease remains caller-owned. The weak-reference finalizer is inert during ordinary collection; at interpreter exit it routes a still-live watcher through normal stop ownership and never calls thread-local Queue cleanup directly. Generic `TERMINAL_ERROR` remains retry exhaustion. `ERROR_HANDLER_FAILURE` bypasses retry, cleans runtime resources, and propagates the original error-handler exception once with the handler exception as cause. The state uses the established `_run_thread` cleanup-owner slot plus construction-time Queue ownership. |
...
| `SM-REDIS-WRITE` (confirmed) | Redis `core.py::_write_message` plus the process-local target write-lock registry, `scripts.py::WRITE_MESSAGE`, persisted high-water, row indexes, and optional keep reservation guard; realizes `[SB-ID-2]` and `[SB-DELIVERY-9]` | `REDIS_WRITE_TRANSITIONS` | `extensions/simplebroker_redis/tests/test_redis_state_machine_transitions.py::test_redis_write_fires_transition_table`; real-Valkey same-target serialization, stale-fence, visibility, monotone-resync, command-count, keep-window, corrupt-body, and reservation tests in `test_redis_atomicity.py` | Process and target identity, the shared write lock, locally reserved candidate, server high-water, optional displaced set, reservation state, Lua result, and shared conflict count govern serialization, fork reset, refresh, monotone resync, retry, commit, or terminal failure. Ordinary success and keep success with zero or many displaced rows share one script. Namespace failure, corrupt displaced body, active reservation conflict, timestamp conflict, transport ambiguity, and retry exhaustion are explicit outcomes. High-water, row publication, and keep claims share the successful Lua visibility point; Pub/Sub and maintenance remain post-commit. |
