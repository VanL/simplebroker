...
| Common Patterns `Resilient Worker with Checkpointing` block (orig 642–715) | deleted-as-duplicate: `examples/resilient_worker.sh:58–86` preserves both unique statements — temp-file+rename atomic checkpoint (lines 83–84) and per-message acknowledge-by-delete (line 80); README keeps 3-line pointer | TB.4 | Phase B slice review |
