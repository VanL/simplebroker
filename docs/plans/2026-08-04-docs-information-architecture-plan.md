...
| Plan scope: relocation only (no spec) | "32 config keys" Q&A sentence moved verbatim | Corrected to 31 in both guide occurrences | Completion-review round 2 counted `load_config()` (31 keys, reproduced by author: `len(load_config()) == 31`); the original README number was factually wrong. Human-catalog claim, not a registered concern clause | n/a — factual correction, no contract text |
...
| PC1 | External agent comment via owner (post-completion) | P2 | Three routing surfaces (llms.txt, README, CHANGELOG) promised a "full `BROKER_*` catalog" while the guide documented 19 of 31 keys, omitting observable settings (generator batching, watcher burst/jitter/idle, diagnostics, backend fields) | **Accepted, fixed — resolution 1 (document all 31).** Guide gains Watcher additions, Generator Batching, Diagnostics, and Backend Selection groups with defaults sourced from `_constants.py` docstrings; intro states where the remaining 3 keys are documented; completeness verified mechanically (`all k in guide for k in load_config()`). The "full catalog" routing claims are now true rather than weakened |
| CR1–CR3 | Codex (completion review; invocation hit the 540s timeout after reporting findings in narrative, before the verdict table — recorded per call-agent §4; findings actioned rather than rerun blindly) | P2×3 | CR1: three migration-introduced README relative links break on PyPI (D2/invariant 9); CR2: README/guide claimed a "full 32-key / every variable" catalog while listing 18; CR3: selection concern-row still named the removed Checkpoint-based Processing locus | **All accepted, fixed** in the closure commit: links absolutized; catalog claims reworded to documented-keys language with the 32-total context sentence; registry cell now names Filtering by message id. Scoped verification round follows |
| SB-R2–R4 | Codex (slice rounds 2–4) | P3→nit | Round 2: stale ledger line citations; BATCH_SIZE unvalidated. Round 3: `00`/empty edge cases. Round 4: comment claimed no upper bound but out-of-int64 values are rejected | **Accepted, fixed** (`52eb85e`, `91a153e`, comment fix). Loop closed at round 4 by author disposition: the final finding is comment accuracy on a demo knob, behavior is correct (author probes: abc/0/00 rejected, 007 accepted, out-of-range rejected with the same error path); a fifth verification round would be ceremony (review-loops: findings that remove machinery count, blocking is disposition-time) |
...
| `## Features` + `## Use Cases` (orig 103–133) | folded into README §How SimpleBroker Thinks + §Recommended For (every bullet's content restated: zero-config/ACID/concurrency/portability/watcher/CLI in merged bullets; throughput updated to measured ~1,700 ops/s per owner instruction; ownership-boundary paragraph moved verbatim into the theory section; Not-for line retained) | TC.1 | Phase D completion review |
| Architecture `<details>`: Design Philosophy (four rules) | folded into README §How SimpleBroker Thinks (condensed restatement; full original wording no longer needed — the rules' normative content lives in `[SB-API-*]`/`[SB-CLI-*]` and the guides) | TC.1 | Phase D completion review |
| `## Architecture & Technical Details` section shell + pointer paragraph | removed; its three pointers (implementation internals, security, backends) relocated to README §Going Further router table | TC.2 | Phase D completion review |
| Common Patterns `Multiple Queues` block | deleted-as-duplicate: README Quick Start already demonstrates multi-queue writes, `list`, and `list --stats` with the same commands | TC.3 | Phase D completion review |
| README length note | Final 818 lines vs the plan's soft "≤ ~750" stop gate — accepted: remaining sections are catalogs, bound content, and orientation the plan assigns to the README | TC | Phase D completion review |
