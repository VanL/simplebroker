...
- `claim_many()` and `move_many()` are materialized list APIs; they commit before returning their result lists.
- Retry-on-stop `at_least_once` semantics remain available only on generator-based APIs:
  - `claim_generator()`
  - `move_generator()`
...
- For compatibility, keep the `delivery_guarantee` parameter on list-returning APIs in this change set, but:
  - treat all list-returning paths as exactly-once materialization
  - accept `delivery_guarantee="at_least_once"` without warning because exactly-once materialization satisfies the weaker delivery-to-caller contract
  - update docs/docstrings to steer users to generator APIs for retryable processing semantics
...
The list-returning batch APIs expose a `delivery_guarantee` choice that does not create distinct processing-retry behavior. The documentation must make that boundary explicit.
...
Add tests that pin the supported compatibility behavior.

Required tests:

1. **`BrokerDB.claim_many(..., delivery_guarantee="at_least_once")` behaves exactly like default**
   - Assert returned bodies/timestamps are still correct.
   - Assert claimed messages are gone immediately after the call returns.

2. **`BrokerDB.move_many(..., delivery_guarantee="at_least_once")` behaves exactly like default**
   - Assert the source queue loses the moved messages.
   - Assert the destination queue gains them.

3. **Queue wrapper: `Queue.read_many(..., delivery_guarantee="at_least_once")` is accepted**
   - This confirms the public Queue API surfaces the same compatibility behavior.

4. **Queue wrapper: `Queue.move_many(..., delivery_guarantee="at_least_once")` is accepted**
...
- `claim_many()`:
  ...
  - accept `delivery_guarantee="at_least_once"` as satisfied by exactly-once materialization
- `move_many()`:
  - same behavior
- `Queue.read_many()` / `Queue.move_many()`:
  - keep parameter as supported API
  - continue delegating
...
- `delivery_guarantee="at_least_once"` is supported and satisfied by exactly-once materialization
- users should switch to generator APIs for retry-on-stop `at_least_once` processing
...
Only continue when the list-returning APIs accept `at_least_once` and the generator APIs still behave correctly.
...
3. **Batch list APIs**
   ...
   - Mention that passing `delivery_guarantee="at_least_once"` is supported and satisfied by exactly-once materialization.
...
- P2:
  - list-returning APIs no longer pretend to expose generator-style retry semantics
  - non-default `delivery_guarantee` is accepted and satisfied by exactly-once materialization
