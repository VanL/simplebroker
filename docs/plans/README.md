...
| Plan | Status |
|------|--------|
| 2026-08-25-closeable-queue-iterator-contract-plan.md | active - class 5; owner-authorized atomic promotion of the existing owner-thread close capability for read, move, and stream Queue iterators through public types, one local high-level move close-forwarding fix, compact real-backend lifecycle proof, and aligned contracts/docs |
| 2026-08-25-test-suite-audit-remediation-plan.md | draft — class 4; full-suite audit remediation in eight slices: CI-flake mechanics (unscaled valves, stdlib-global patches, Windows subprocess volume), cannot-fail deletions, procedure-reification rewrites, prose/double-entry gate refits, redundancy collapse, and extension-workflow timeout bounds; awaiting owner review before execution |
