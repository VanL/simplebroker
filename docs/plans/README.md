...
| Plan | Status |
|------|--------|
| 2026-08-27-all-examples-correctness-and-contract-alignment-plan.md | completed - class 4; updated and behaviorally verified the complete examples inventory for the v8 contract, removed example-owned schema DDL, repaired async ordering/stream loss, reactor late-lower-ID discovery, watcher delivery claims, and shell partial-failure handling, and passed release-readiness gates without changing product specs or publishing a release |
| 2026-08-27-message-id-order-and-newest-selection-plan.md | active - class 5+P; re-contracts uniform retrieval as public-message-ID order, adds bounded Python `order="newest"` and CLI `--newest`, migrates fresh and v5 SQL targets to one supported surrogate-free v6 broker-owned layout while preserving caller sidecars, requires a quiesced coordinated cutover across SimpleBroker, Weft, and Taut (normal old target opens refuse v6 cleanly; the PG 3.10.0 injected-runner path is an excluded legacy exception), serializes PostgreSQL migration with an advisory lock and live version recheck, and retains rollback, exact-artifact, and independent-review gates |
| 2026-08-26-release-gate-startup-failure-recovery-plan.md | completed - class 3; v7.5.0 remains an unpublished immutable incident tag, future-tag recovery preserves tag identity and Trusted Publishing boundaries, and exact-SHA v7.5.1/PG/Redis gates published verified immutable releases after the GitHub Actions outage |
| 2026-08-25-postgres-connection-pressure-inspection-plan.md | completed - class 5; independently reviewed Queue backend identity and PostgreSQL `numbackends` pressure inspection shipped in simplebroker 7.5.1 and simplebroker-pg 3.10.0 with real PG15/18 proofs, exact-SHA CI, clean public-artifact install, corrected Weft handoff, and immutable publication evidence |
| 2026-08-25-schema-and-representation-assumption-remediation-plan.md | completed — class 5+P; corrected single-connection SQLite admission/proof and FIFO, backend ownership/readiness, project-scoped service PhaseLock coordination, path/config/session and developer-tool representation assumptions; full core, live PostgreSQL/Redis, Weft, packaging, static/docs, benchmark, and independent review gates passed with backend API v7 preserved |
