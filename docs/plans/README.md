...
| Plan | Status |
|------|--------|
| 2026-09-11-shared-configuration-loader-plan.md | completed — class 5; additive shared extensible loader, canonical internal values with compatible namespaced views; existing public behavior and precedence preserved; SimpleBroker-only implementation, Weft/Taut reference only; verification and independent review passed, additional external review timed out; closed in the owner-authorized implementation commit |
...
|------|--------------|---------|---------------|------------|
| 2026-08-27-all-examples-correctness-and-contract-alignment-plan.md | 2026-08-28 | Aligned the full examples catalog with the v8 message-ID, lifecycle, and error contracts; removed example-owned schema DDL and repaired async, reactor, watcher, and shell correctness hazards. | `[SB-CLI-*]`, `[SB-DELIVERY-*]`, `[SB-ID-*]`, `[SB-SELECT-*]`, `[SB-IO-*]`, `[SB-API-*]`, `[SB-OPS-*]`; example guides and behavior suites | `813dd7ce` |
| 2026-08-26-release-gate-startup-failure-recovery-plan.md | 2026-08-28 | Recovered exact-SHA publication after a GitHub Actions outage while preserving the unpublished v7.5.0 incident tag, Trusted Publishing boundaries, and immutable tag identity. | release workflows; `bin/release.py`; public artifact and tag evidence; incident sequence judged plan-local | `813dd7ce` |
| 2026-08-25-postgres-connection-pressure-inspection-plan.md | 2026-08-28 | Shipped Queue-scoped PostgreSQL connection-pressure inspection with PG15/18, lifecycle, clean-install, and immutable-release proof. | `[SB-API-13]`; backend guide and PostgreSQL README; connection-stats code and tests | `813dd7ce` |
| 2026-08-25-schema-and-representation-assumption-remediation-plan.md | 2026-08-28 | Corrected storage admission/proof, backend readiness, service setup coordination, path/config/session identity, and developer-tool representation assumptions without changing backend API v7. | `[SB-CLI-*]`, `[SB-DELIVERY-*]`, `[SB-ID-*]`, `[SB-SELECT-*]`, `[SB-API-*]`; implementation docs 06/07/09; dated 2026-08-24 and 2026-08-26 lessons; code/tests | `813dd7ce` |
| 2026-08-25-shared-backend-proof-remediation-plan.md | 2026-08-28 | Restored the intended `shared and not sqlite_only` topology and proved 59 promoted contract nodes across SQLite, PostgreSQL, and Redis. | `[THEORY-2/3]`, `[SB-API-11]`, implementation doc 06; backend wrapper and contract-node suites; 2026-08-28 evaluated-marker lesson | `813dd7ce` |
| 2026-08-25-verified-review-findings-remediation-plan.md | 2026-08-28 | Closed eight verified findings across admission, lifecycle, diagnostics, vacuum failure order, delete parity, and exact timestamp bounds. | `[SB-CLI-*]`, `[SB-DELIVERY-*]`, `[SB-ID-*]`, `[SB-IO-*]`, `[SB-API-*]`, `[SB-OPS-*]`; implementation docs 06–10; code/tests | `813dd7ce` |
| 2026-08-25-closeable-queue-iterator-contract-plan.md | 2026-08-28 | Made existing owner-thread cleanup for read, move, and stream iterators public through `CloseableIterator` without widening foreign-thread recovery. | `[SB-DELIVERY-5/6]`, `[SB-API-1/4/5]`; implementation docs 04/06; Python guide and lifecycle tests | `813dd7ce` |
| 2026-08-25-test-suite-audit-remediation-plan.md | 2026-08-28 | Completed the test-suite audit with owned fault seams, causal concurrency proofs, equivalence-based deletion, stronger contract gates, and explicit direct/wrapper CI topology. | 2026-08-25 testing lessons; implementation docs 07/10; test infrastructure, workflow gates, and firing mutations | `813dd7ce` |
| 2026-08-24-cli-grammar-validation-and-example-reliability-plan.md | 2026-08-28 | Corrected CLI option grammar, pre-target validation, action JSON, typed default derivation, shell examples, parser fuzzing, and CI/ShellCheck reliability. | `[SB-CLI-3/4/5]`, `[SB-SELECT-*]`, `[SB-API-6]`; implementation doc 07; 2026-08-23 preparse lesson; CLI/example tests | `813dd7ce` |
| 2026-08-24-cli-output-and-error-contract-remediation-plan.md | 2026-08-28 | Unified all exported command functions on direct exceptions and outer CLI translation while pinning selector parity and delete no-mutation behavior. | `[SB-CLI-1/2/4]`, `[SB-API-10]`; implementation doc 07; structured-output and error-ownership tests | `813dd7ce` |
| 2026-08-24-comprehensive-review-findings-remediation-plan.md | 2026-08-28 | Corrected 15 review findings through their existing owners and transferred the two follow-up findings to plans that later closed. | product specs 10/11/13/16; implementation docs 06/07/09; successor CLI and test-audit outcomes; code/tests | `813dd7ce` |
| 2026-08-24-failure-path-and-contract-findings-resolution-plan.md | 2026-08-28 | Made watcher callback failures terminal, repaired CLI/context, peek/delete, and cleanup contracts, and retained the measured no-index vacuum decision. | `[SB-CLI-*]`, `[SB-DELIVERY-*]`, `[SB-ID-3]`, `[SB-API-*]`, `[SB-OPS-*]`; implementation docs 07/09; `[ALT-IMPL09-001]`; code/tests | `813dd7ce` |
| 2026-08-24-phaselock-status-temp-ownership-plan.md | 2026-08-28 | Put stale fallback-marker and temp-file cleanup under the phase-lock owner across status and xattr modes. | implementation doc 09; 2026-08-24 cleanup-ownership lesson; `_phaselock.py`, runner, and lifecycle tests | `813dd7ce` |
| 2026-08-24-peek-generator-close-contract-plan.md | 2026-08-28 | Made peek iterators closeable with same-thread synchronous Queue-operation cleanup; the successor generalized the same capability to read, move, and stream. | `[SB-DELIVERY-4/6]`, `[SB-API-1/4/5]`; implementation docs 04/06; closeable-iterator successor; lifecycle tests | `813dd7ce` |
