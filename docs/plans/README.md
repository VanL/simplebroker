...
| Plan | Status |
|------|--------|
| 2026-08-06-pre-release-review-remediation-plan.md | active — initial implementation committed at `a38e6a9`; first Windows CI remediation committed at `4946dfd`; class 5; revision 6 A/E/I/J passed full root, PostgreSQL, Redis, explicit probe, static, interface, and independent re-review gates on 2026-08-07; second Windows remediation is confirmed green on all four Windows jobs; three release attempts correctly stopped before tags on watcher readiness, backend marker scope, then a scheduler-based Windows assertion plus a coverage/Python SIGTERM re-entry deadlock; deterministic test fixes passed the exact full coverage shape and final independent review; B is operator-owned permission documentation; C/D/F/G/H deferred; no tag/publish; K0/K remain release-time stop gates |
