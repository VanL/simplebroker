...
Authority is defined by `product-section-registry.md`. **All current product
concern families are `canonical-spec`** (`10-cli.md` … `17-ops.md`).

State vocabulary (for future concerns and historical transitions):

- `readme-only` — no canonical product section yet; root `README.md` is
  normative for that concern until promotion.
- `draft-spec` — scaffold under review; root `README.md` still wins on
  conflict until `canonical-spec`.
