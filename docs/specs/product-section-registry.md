...
| Dump/load and claimed-row I/O | `canonical-spec` | `15-persistence-io.md` `[SB-IO-1]`…`[SB-IO-5]` | README dump/load + include-claimed (reduced); agent-kernel Dump/load | `tests/test_persistence_io_contract_sb_io.py`; routine `extensions/simplebroker_pg/tests/test_pg_dump_load_pipe.py` and `extensions/simplebroker_redis/tests/test_redis_dump_load_pipe.py`; opt-in direct PostgreSQL↔Redis `tests/test_cross_backend_dump_load.py`; claimed inspection `tests/test_peek_include_claimed.py` |
