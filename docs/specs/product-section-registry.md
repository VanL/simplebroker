...
| Queue and broker residual operations | `canonical-spec` | `17-ops.md` `[SB-OPS-1]`…`[SB-OPS-7]` | README Command Reference (metadata, delete, rename, aliases, vacuum, destructive target cleanup); Python API metadata | `tests/test_operations_contract_sb_ops.py`; `tests/test_queue_metadata.py`; `tests/test_cli_queue_metadata.py`; `tests/test_queue_rename.py`; `tests/test_cli_rename.py`; `tests/test_aliases_db.py`; `tests/test_alias_cli.py`; `tests/test_batch_delete.py`; `tests/test_delete_from_queues.py`; `tests/test_custom_runner_integration.py`; `tests/test_queue_api_additions.py`; `tests/test_queue_api_comprehensive.py`; `tests/test_safety_fixes.py`; `tests/test_vacuum_compact.py`; `tests/test_maintenance_policy.py`; `tests/test_constants.py`; `tests/test_cleanup.py`; `tests/test_cli_argument_parsing.py`; `extensions/simplebroker_redis/tests/test_redis_atomicity.py` (flat concurrent alias mutation and partial delete-all truth) |
