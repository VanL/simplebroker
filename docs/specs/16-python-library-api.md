...
their own last close. On the main thread, the cached core is released only when
...
remains live. Non-ordinary `BaseException` behavior retains the existing
propagation priority and failed-core ownership. `GeneratorExit` raised to close
