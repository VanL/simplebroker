...
active run owns runtime cleanup in its established `finally`; an idle internally
constructed Queue follows its own Queue finalizer, while a caller-supplied Queue
retains the caller's ordinary lifetime and stop state.
