...
def test_checkpointed_control_is_not_reprocessed_after_restart(tmp_path: Path) -> None:
    ...
    try:
        ...
        _send_control(db_path, "STOP", "stop-first")
        _wait_for_control_reply(db_path, request_id="stop-first")
        thread.join(timeout=2.0)
        assert not thread.is_alive()
    finally:
        reactor.stop()
...
def test_pending_output_rows_replay_on_reactor_construction(tmp_path: Path) -> None:
    db_path = tmp_path / "reactor.db"
    ...
    try:
        ...
        assert ({"replayed": True}, output_message_id) in outputs

        status_queue = Queue(INBOX_A, db_path=str(db_path))
        with status_queue.sidecar() as session:
            rows = list(
                session.run(
                    """
                    SELECT status
