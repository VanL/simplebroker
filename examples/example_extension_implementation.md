...
class DaemonRunner(SQLRunner):
    ...
    def _start_daemon(self):
        ...
        self._daemon = threading.Thread(
            target=self._daemon_loop, daemon=True, name=f"DaemonRunner-{self.db_path}"
        )
        self._daemon.start()
    ...
    def _execute_sql(
        self, conn, sql: str, params: tuple[Any, ...] = (), *, fetch: bool = False
    ) -> Iterable[tuple[Any, ...]]:
...
class AsyncQueue:
    ...
    async def read(self) -> Optional[str]:
        ...
        try:
            ...
            # Delete the message
            await self._runner.run("DELETE FROM messages WHERE id = ?", (msg_id,))
...
async def main():
    ...
    async with AsyncQueue("async_tasks", runner) as q:
        ...
        await asyncio.gather(
            q.write("Async task 1"), q.write("Async task 2"), q.write("Async task 3")
        )
...
def test_transaction_rollback(queue_with_mock, mock_runner):
    ...
    mock_runner.set_error_on_next_call(sqlite3.IntegrityError("Simulated error"))
...
def test_performance_tracking(queue_with_mock, mock_runner):
    ...
    # Count SQL calls
    write_calls = sum(
        1 for call in mock_runner.call_history if "INSERT INTO messages" in str(call)
    )
...
class AsyncBrokerCore:
    ...
    async def stream_read(self, queue: str, batch_size: int = 10) -> AsyncIterator[str]:
    ...
    async def queue_size(self, queue: str) -> int:
        ...
        cursor = await self._conn.execute(
            "SELECT COUNT(*) FROM messages WHERE queue = ? AND claimed = 0", (queue,)
        )
        row = await cursor.fetchone()
...
async def example_concurrent():
    ...
    async with async_broker() as broker:
        ...
        await asyncio.gather(producer(1), producer(2), consumer(1), consumer(2))
